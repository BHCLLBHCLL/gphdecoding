#!/usr/bin/env python3
"""
FPH to CGNS Converter.

Reads FPH (CRDL-FLD) binary format produced by Software Cradle scFLOW / SCTpre.
FPH is a superset of GPH: it contains the same mesh sections (LS_Nodes,
LS_Links, LS_Parts, …) plus solver result data (pressure, velocity, …) stored
in the ``LS_SPHFile`` section.

Writes a CGNS/HDF5 unstructured mesh file using the NGON_n / NFACE_n
...
def _f32_be_array(buf, offset: int, count: int) -> np.ndarray:
    """Read *count* big-endian float32 values from *buf* at *offset*."""
...
def _parse_ls_surface_regions(data: bytes,
    ...
    *n_faces* (optional) is the total number of faces in the mesh.  When
    provided, blocks whose parsed "face IDs" exceed ``n_faces`` are
    rejected — this filters out non-face-ID blocks (e.g. material name
    strings like ``"air(incompressible)"`` that some FPH exporters emit
    inside ``LS_SurfaceRegions`` under entries such as
    ``LS_MaterialOfParts``).
    """
    ...
    while i + 2 < len(blocks):
        ...
        if bc_i > 0 and bc_i == bc_w and bc_i % 4 == 0:
            # Reject blocks that are clearly ASCII text rather than I4 face
            # IDs.  Some FPH exporters interleave material-name strings
            # (e.g. "air(incompressible)", "copper(Cu)(300K)") inside
            # LS_SurfaceRegions; these happen to be 4-byte-aligned and so
            # would otherwise be misparsed as huge out-of-range face IDs.
            payload = data[p_i : p_i + bc_i]
            printable = sum(1 for b in payload if 32 <= b < 127)
            if bc_i >= 4 and printable >= bc_i * 3 // 4:
                i += 3
                continue
            face_ids = np.frombuffer(payload,
                                     dtype=">i4").astype(np.int64).copy()
            # Bounds-check: face IDs must be valid 0-based indices into the
            # global face array.  Out-of-range blocks are material metadata
            # or other non-face-ID payloads and are skipped.
            if n_faces is not None and face_ids.size:
                if face_ids.min() < 0 or face_ids.max() >= n_faces:
                    i += 3
                    continue
...
# ─────────────────────────────────────────────────────────────────────────────
# LS_SPHFile – solver result data (FPH only)
# ─────────────────────────────────────────────────────────────────────────────
#
# FPH files append a ``LS_SPHFile`` section after the mesh metadata.  It
# contains the solver output as a sequence of variable records:
#
#   [32B ASCII name]   e.g. ``EC_Scalar:PRES``  /  ``EC_Vector:VEL``
...
#   EC_Scalar:NAME  ->  ``NAME``
#   EC_Vector:NAME  ->  ``NAMEX``, ``NAMEY``, ``NAMEZ``
#
# The vendor exporter writes scalars first (in file order), then vectors
# (in file order), so we preserve that ordering.
# ─────────────────────────────────────────────────────────────────────────────


def _parse_fph_flow_solution(data: bytes, n_cells: int) -> "dict[str, np.ndarray]":
    """Parse ``LS_SPHFile`` and return cell-centered result variables.

    Returns a dict mapping CGNS variable names to ``float64`` arrays of shape
    ``(n_cells,)``.  Returns an empty dict when the section is absent (e.g.
    plain GPH files with no solver data).
    """
    sec_start = _find_section(data, "LS_SPHFile")
    if sec_start < 0:
        return {}
    sec_end = _section_end(data, sec_start)
    ...
    # Locate variable-name blocks (32B ASCII starting with EC_Scalar:/EC_Vector:).
    ...
    for vi, (bi, name) in enumerate(name_indices):
        ...
        # Collect data blocks of the expected float32 size between this name
        # and the next variable name.
        data_blocks: list[np.ndarray] = []
        for j in range(bi + 1, next_bi):
            p, bc = blocks[j]
            if bc == expected_data_bytes:
                arr = np.frombuffer(data[p:p + bc],
                                    dtype=">f4").astype(np.float64).copy()
    ...
    # Vendor convention: scalars first (file order), then vectors (file order).
...
def parse_gph_mesh(filepath: str) -> dict:
    ...
    try:
        ...
        result["surface_regions"] = _parse_ls_surface_regions(
            data,
            n_faces=int(link_data["n_faces"]) if link_data else None,
        )
        ...
        # FPH result data (LS_SPHFile).  Parsed after the mesh so we know
        # n_cells.  Empty for plain GPH files (no LS_SPHFile section).
        result["flow_solution"] = _parse_fph_flow_solution(data, link_data["n_cells"])
...
# ParaView's vtkCGNSReader cannot parse, resulting in a
# "Failed to parse cgns file" error.
#
...
def _create_compact_group(parent, name: str):
    ...
    the object header).  This is critical for ParaView's vtkCGNSReader: the
    default old-style group format (Symbol Table only) writes a malformed
    root object header that ParaView cannot parse.
...
def _build_zone_plan(mesh: dict,
    ...
    for part, cvol_spec in parts_with_cvol:
        ...
        # Skip parts that have no cells in this mesh (e.g. a solid part
        # whose cvol_id is absent from LS_CvolIdOfElements).  An empty
        # zone would produce degenerate NGON/NFACE arrays that some CGNS
        # readers (including ParaView) reject.
        if not mask.any():
            continue
...
def write_cgns(mesh: dict, outpath: str,
    ...
    * ``Base`` (CGNSBase_t, I4 ``[3, 3]``)
    ...
    # vtkCGNSReader cannot parse.  The FLDUTIL reference file also uses
    # new-style groups with link creation order tracking.
    with h5py.File(outpath, "w", libver=("earliest", "v108"),
        ...
        flow_solution = mesh.get("flow_solution") or {}
...
def main():
    ...
    parser.add_argument("-z", "--zone", default=None,
                        help="When --single-zone is given, override the zone name "
                             "(default: 'FluidRegion').")
    parser.add_argument("--clip-flow", type=int, default=0, metavar="0|1",
    ...
    # Optionally clip sentinel values (> 1e20) in FlowSolution arrays.
    flow_solution = mesh.get("flow_solution") or {}
    if args.clip_flow == 1 and flow_solution:
        clip_threshold = 1e20
        for var_name, arr in flow_solution.items():
            mask = arr > clip_threshold
