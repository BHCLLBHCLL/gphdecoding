...
**cvol_id 扫描 sanity check（#14 / #16 / #17）**：与 `LS_CvolIdOfElements` 进行**逐元素**交叉校验：

- 字节扫描得到的每个 cvol_id 都必须出现在单元实际引用的 cvol_id 集合（即 `LS_CvolIdOfElements` 的唯一值集合）中。
- **逐个验证**：对每个 Part 单独判定；扫描值**在**集合内则信任并保留（即便它非连续，例如 `{1, 9, 11}`），**仅当**扫描值**不在**集合内时该条目才回退为其在 `LS_Parts` 中的 1-based 顺序索引（`i+1`）。
- 该机制能精准中和那些 latch 到无关 `[12,4,X,4]` 描述符的条目，同时不丢弃其他扫描正确的 id；不再依赖"首值必为 1"或"多数命中"的聚合启发式。
- 当 `LS_CvolIdOfElements` 不可用时无法交叉校验，扫描出非 `None` 即予信任，否则回退为顺序索引。

`gph2cgns.py` 的 `_parse_ls_parts_with_cvol_ids(data, cvol_id=…)` 与 `gph_model.py` 的 `parse_ls_parts(data, cvol_id=…)` 均接受 `cvol_id` 形参以执行该逐元素校验，行为保持一致。
