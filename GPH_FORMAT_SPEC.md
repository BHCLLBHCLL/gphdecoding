...
| LS_Links | I4[] | 连接表（大端 int32，单元连接） |
| LS_Nodes | R8[n,3] | 顶点坐标（字反转 float64，三轴分块存储） |
| LS_SurfaceRegions | 变长 | 表面区域定义 |
| Element_InformationFlag | 变长 | 单元信息标志 |

### LS_Nodes 详细格式

节点坐标按 X / Y / Z 三个独立轴块存储。**实际格式有两种方言**，需在运行时根据数值幅值自动判别：

#### 方言 A：旧版 SCTpre（字反转 float64）

```
[16B 描述符] 00 00 00 0C / 00 00 00 08 / n_verts / 00 00 00 01
[12B 块头]   00 00 00 0C / byte_count  / upper_half_of_max_coord
[n_verts × 8B]  字反转 float64
```

- 轴顺序：**X, Z, Y**（解析后需重排为 X, Y, Z）
- 块间无尾部哨兵；下一块描述符紧接其后

**字反转 float64**：每个 8 字节双精度浮点数按 `[低32位字(大端)][高32位字(大端)]` 顺序存储。

示例：坐标值 0.01 = `0x3F847AE147AE147B`，在文件中存为字节 `47 AE 14 7B 3F 84 7A E1`。

若错误地按两个 float32 读取，会得到 `(89128.96, 1.035)`——即"89000+"坐标 bug 的根本原因。

#### 方言 B：新版 ANSA / scFLOW（标准大端 float64）

```
[16B 描述符] 00 00 00 0C / 00 00 00 08 / n_verts / 00 00 00 01
[8B  块头]   00 00 00 0C / byte_count
[n_verts × 8B]  标准大端 float64
...
示例：坐标值 `float32(0.01)` 加宽为 float64 = `0x3F847AE140000000`，在文件中以标准大端写为 `3F 84 7A E1 40 00 00 00`。

#### 方言自动判别

`gph2cgns.py` / `gph_model.py` / `gph_parser.py` 均会同时按两种方式解析，选择幅值物理合理（`1e-30 ≤ |v| ≤ 1e6` 或 `v == 0`）的结果。
