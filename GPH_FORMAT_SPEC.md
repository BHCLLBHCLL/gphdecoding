...
**cvol_id 扫描 sanity check（#14）**：经验上首个 Part 的 cvol_id 在合法文件中恒为 `1`（`box_ansa`: `[1]`，`tr03`: `[1,2]`，`laptop`: `[1, 9, 11]`）。若描述符字节扫描返回首值 `>1`（说明 latch 到了无关 `[12,4,X,4]` 描述符），整个扫描视为不可信，解析器回退为顺序 1-based 编号（`1, 2, 3, …`）；首值为 `1` 时信任扫描结果，保留 `{1, 9, 11}` 等合法的非连续 id。`gph2cgns.py` 与 `gph_model.py` 实现一致。
