...
… 若干 post-name 描述符 …
[12, 4, 1, 4]           ← marker（常见）
[12, 4, <cvol_id>, 4]   ← 该 Part 的 cvol_id（下一 Part 名块之前）
```

**实测 post-name 描述符链**：合法文件中每个 Part 名块之后通常出现 **`[1, cvol_id]`** 形式的 `[12,4,X,4]` 序列——前导 `1` 为 marker，末尾值为 scFLOW 侧的不透明 Part id。例如：
...
| `laptop_simplified.gph` | air_domain / rotation1 / rotation2 | `[1,1]` / `[1,4]` / `[1,2]` | 1 / 4 / 2 |

**cvol_id 解析（统一算法）**：`gph_model.parse_ls_parts(data, cvol_id=…)` 为唯一实现；`gph2cgns.py` 直接导入该函数（别名 `_parse_ls_parts_with_cvol_ids`），与 `gph_parser.py` / `gphviewer.py` 行为一致。

1. 先解析 `LS_CvolIdOfElements`，取其唯一值集合 $S$ 作为权威 cvol_id 集合（未提供 `cvol_id` 参数时跳过校验）。
2. 对每个 Part 名块，扫描其后至下一 Part 名块之间的**完整** `[12,4,X,4]` 描述符链。
3. **主规则**：取链中**最后一个属于 $S$ 的值**作为该 Part 的 cvol_id（而非无条件取最后一个 X——避免 marker `1` 与真实 id 混淆时的误读）。
4. **全局校验**：映射必须唯一；当 $|S| = N$（Part 数）时须完全覆盖 $S$。
5. **回退**（按序）：(a) 每 Part 链中恰有一个值 ∈ $S$ → 用之；(b) $S = \{1,\dots,N\}$ → 顺序编号；(c) 链末元素；(d) 主规则 best-effort。

> **历史**：§11.9 / §11.10 曾用「首值必须为 1」+「多数交叉校验」+ 顺序回退 $1..N$ 的双启发式；对 `{1,9,11}` 等非连续 id 存在误触发顺序编号的风险，已由上述全局链扫描算法取代（见 `DEV_SUMMARY.md` §11.11）。
