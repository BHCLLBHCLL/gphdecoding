...
| 0x26B0-0x2C60 | 变长 | LS_Nodes | R8[n,3] 顶点坐标（三轴 float64 块，§5.3）|
...
| LS_Nodes | R8[n,3] | 顶点坐标（三轴 float64 块，§5.3）|
...
`gph2cgns.py` 用 `LS_Parts` 中每个 Part 名块后的 `[12, 4, cvol_id, 4]` 描述符建立 `part_name → cvol_id` 映射，再与 `LS_CvolIdOfElements` 匹配生成各 Zone 的 cell 子集。
...
节点坐标按 X / Y / Z 三个等长 float64 轴块存储（经块扫描取三个最大且等长的 f64 块）。
...
`gph2cgns.py` / `gph_model.py` / `gph_parser.py` 扫描节内三个等长 float64 块，对标准大端与字反转两种读法打分，选取物理合理者；字反转选中时对磁盘轴序 X,Z,Y 做 Y/Z 交换。
...
ASCII 字符串块列表，顺序即 CGNS 中体区域 Zone 的生成顺序（如 `FluidRegion`、`Rotate_MovingVolumeRegion` 等）。`gph2cgns` 按名称与 `LS_CvolIdOfElements` / Part 启发式匹配 cell 子集。
...
[trailer=255]
… 若干描述符 …
[12, 4, <cvol_id>, 4]   ← 该 Part 的 cvol_id（在下一 Part 名块之前）
```

**cvol_id 扫描 sanity check（#14 / #16）**：两条互补的启发式规则共同判定扫描结果是否可信：

1. **首值规则**：经验上首个 Part 的 cvol_id 在合法文件中恒为 `1`（`box_ansa`: `[1]`，`tr03`: `[1,2]`，`laptop`: `[1, 9, 11]`）。若描述符字节扫描返回首值 `>1`（说明 latch 到了无关 `[12,4,X,4]` 描述符），整个扫描视为不可信，解析器回退为顺序 1-based 编号（`1, 2, 3, …`）。
2. **与 `LS_CvolIdOfElements` 的交叉校验**：扫描得到的每个 cvol_id 都必须出现在单元实际引用的 cvol_id 集合（即 `LS_CvolIdOfElements` 的唯一值集合）中。若**多数**扫描值不在此集合内，扫描视为不可信，同样回退为顺序 1-based 编号。该规则用于大型/重存模型——单纯依赖首值规则不够（首值可能恰好为 `1`，而后续 Part 的扫描值仍为垃圾）。

首值为 `1` 且多数扫描 id 都属于实际集合时，扫描结果被信任，保留 `{1, 9, 11}` 等合法的非连续 id。`gph2cgns.py` 的 `_parse_ls_parts_with_cvol_ids(data, cvol_id=…)` 与 `gph_model.py` 的 `parse_ls_parts(data, cvol_id=…)` 均接受 cvol_id 参数以执行该交叉校验，行为保持一致。
